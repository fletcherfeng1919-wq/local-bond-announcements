...
最后更新：2026-08-11，最新 commit `8eb07e4`（已 push 到 `origin/main`，working tree 干净）。
...
**2026-08-11 追加的一轮修改**（commit `8eb07e4`）：用户拿 Wind 终端截图对比，指出即使补了8月数据，我们的"已确认"覆盖范围（当时到8-07）还是比 Wind 差一大截——Wind 对8-10的招标已经有确认利率了。查证后确认这**不是抓取问题**：直接绕过缓存重新抓 celma 的"发行结果"栏目，最新还是只到8-07——celma 平台自己的确认结果公告本身相对实际招标就有约1周的发布滞后，这是数据源结构性的，不是我们抓取的问题。
...
| GitHub 仓库 | `fletcherfeng1919-wq/local-bond-announcements`，分支 `main`，最新 commit `8eb07e4` |
