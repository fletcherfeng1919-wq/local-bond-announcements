...
最后更新：2026-08-11，最新 commit `ec03064`（已 push 到 `origin/main`，working tree 干净）。
...
- **两轮真实测试后又修了3个具体bug，现在9/10省份能跑**：
  1. **湖南 .docx 已接入**：`pip install python-docx`（已加进 `requirements.txt`），发现湖南的.docx附件内容其实是一个规规矩矩的Word**表格**（不是键值对段落），比PDF/HTML都更好解析——新写了 `parse_docx_table()` 直接按表头列名取值，不走共享的正则分块逻辑。
  2. **重庆结构已确认是"pdf"**：研究阶段WebFetch没能取出附件链接，本次直接用 `requests`+正则重新抓了一次，秒找到PDF——但重庆这份PDF是**全扫描件**（`extract_pdf()` 全走OCR）且版面是"先列一排字段标签，再列一排对应的值"（不是其它省份"标签紧跟着值"的排布），共享正则解析器认这种排布会失败，实测6支债券全部提取不到有效字段（但两层低置信度防护正确拦下了唯一提取出片段数据的1行，没有误报）——**已经如实记录成"结构确认但当前解析器对这份文件低产出"，没有为了迁就这一个省份重写解析器**（性价比低：这份OCR本身质量太差，就算改进分块算法收益也有限）。
  3. **宁波两个格式坑**：① 宁波的"计划/实际发行规模"用**万元**做单位，不是所有其它省份统一的**亿元**——加了 `_AMOUNT_WAN_RE` 兜底（未命中"亿元"格式时再试"万元"格式，➗10000换算）。② 宁波公告正文里根本不用"XXXX年X月X日已完成招标"这句话，招标日期只出现在公告标题里（如"2025年5月23日宁波市政府债券发行结果公告"）——加了 `_TITLE_DATE_RE` 作为 `_ISSUE_DATE_RE` 的兜底。两个坑修完后，宁波那份2025-05-23公告的10支债券全部正确匹配上了celma的记录（此前因为 `total_amount_yi`/`issue_date` 双双解析成 `None`，10支全部落进"找不到匹配"）。
  4. 顺带发现 `czt.hunan.gov.cn` 的封面页HTML本身（不只是.docx附件）也会触发同一个TLS握手失败（`BAD_ECPOINT`），补了 `_fetch_html()` 兜底并把curl兜底结果写回 `http_client` 的磁盘缓存（否则每次调用都要重新shell出去调用curl，实测偶尔会超过30秒超时——已经把curl超时从30秒调到45秒）。
- **最终端到端测试结果**（9个已实现省份，全部真实公告，非模拟数据）：**24支债券干净匹配，0处真实金额/利率差异，6处因源文件本身信息不全无法匹配（5处是重庆的OCR质量问题，1处是江苏OCR漏了期限字段），3处被两层"低置信度"防护正确拦下（新疆×1、河北×1、重庆×1，全部是OCR分块串块，不是真实数据问题）**。**这一轮没有发现任何真实的celma数据错误**——这本身是有价值的信息（对已测试的这几个省份，celma数据得到了独立验证），不是"没做出结果"。
- **仍未接入的1个源**：天津（发行结果直接嵌图片JPG，需要图片OCR，`pdf_extract.py` 现有的OCR路径是针对PDF页面栅格化设计的，改造成吃独立图片文件工作量不小，且从重庆的经验看这类扫描类源本身产出率就偏低）——调用 `verify_announcement('天津市', ...)` 会抛 `NotImplementedError`，不是静默失败。
