...
最后更新：2026-08-12，最新 commit `850b419`（已 push 到 `origin/main`，working tree 干净）。
...
- **chinabond.com.cn 是当前最有潜力的下一步方向**：① 发行计划解析器（第四轮已完成，6/10省补齐，剩4个是源头缺数据）和 OCR质量问题（第四轮已解决，PSM4+pdfplumber渲染，全项目生效）都已经做完；② `check_chinabond_for_sse_gaps()`/`crawl_plans()` 目前都还只是"能读出数据"，如果要往前一步做到"自动patch进state_results.csv/state_plans.csv的置信度"，需要一轮专门的人工抽查校验（挑一批OCR结果和源PDF逐条核对），目前没做；③ chinabond的翻页参数还没找到（`pageSize` 目前只能拿到每个筛选条件下最新的10条），如果要做历史数据回填而不只是"补最近缺口"，需要先破解翻页。
...
| GitHub 仓库 | `fletcherfeng1919-wq/local-bond-announcements`，分支 `main`，最新 commit `850b419` |
