...
最后更新：2026-08-13，最新 commit `c2cab08`（已 push 到 `origin/main`，working tree 干净）。
...
**没有已知阻塞项。** 截至 2026-08-13（commit `f204bcb`），Wind对账、SSE交叉校验、省级财政厅爬虫、chinabond.com.cn全国门户、发行计划解析器都已完成并验证，`git status` 干净。

**三方交叉校验现状**（`three_way_validate.py`，可随时重跑确认未回退）：

| 对比对象 | 窗口 | 差异 |
|---|---|---|
| Wind一级市场导出（最新08-11版） | 近2个月 | 0处差异 |
| SSE上市公告（`check_sse_coverage()`） | 近2个月 | 0条缺口 |
| 8月发行计划（`state_plans.csv`） | 当月 | 4个真实缺口：大连市/河南省/西藏自治区/青岛市，均已确认是数据源本身不发布，非抓取问题（见第五、六轮记录） |

**数据规模**：`state_results.csv` 14,958行（覆盖到2026-08-11）、`state_plans.csv` 2,189行、`state_announcements.csv` 8,390行。

**省级独立数据源**：15个已接入自动爬虫（`provincial_verify.py::PROVINCE_SOURCES` / `provincial_crawl.py::LISTING_URLS`），12个确认是真实死胡同（官网不发布这类内容），2个因环境限制未解决（新疆WAF、天津图片OCR）。

**已知限制（不是待办，是记录在案的边界）**：
1. 2020年前的旧格式结果数据还没有专门解析器（`extract_result_legacy.py` 是占位）。
2. chinabond.com.cn的翻页参数没破解，`pageSize` 只能拿到每个筛选条件下最新10条——只够补最近缺口，不够做历史回填。
3. chinabond结果数据（发行结果channel）的OCR可靠性对部分省份（内蒙古/黑龙江等）仍然有限，`check_chinabond_for_sse_gaps()` 是按需人工核对的诊断工具，没有自动patch进主表的机制——每次出现新缺口都要走"chinabond找文档→渲染成图片肉眼核对→写入"这个人工流程（第五轮已经走过一次，效果不错，值得复用而不是重新摸索）。

### 2026-08-11：Wind核对 + provincial_verify 首次构建（历史记录）

用户提供了一份Wind终端导出的"地方政府债一级市场"报表（覆盖2025-08-11~2026-08-10，2487条已确认债券），说这是最准的数据源，让核对 `state_results.csv` 有没有缺口，顺带把 `provincial_verify.py` 从零建了起来。这条记录留作历史存档——当时发现的问题都已修复、验证、提交，不代表当前状态，当前状态见上面的第3节。

核对结果发现了**三个真实的、系统性的提取bug**（不是孤立的8月问题，全年都有）：
...
### 2026-08-12 第二轮：修复SSE比对的命名bug + 第二次省份调研（10省扩展到14省）

**先修了一个我自己的bug，很重要，别倒退**：`three_way_validate.py::check_sse_coverage()` 原来直接拿SSE的 `securityAbbr` 和celma的 `bond_short_name` 做字符串比对，报告"156条SSE上市公告里107条celma未收录"，看起来是个大缺口。手动核对江苏省的例子后发现**这107条里绝大多数不是真缺口**——SSE用"26江苏31"，celma自己用"26江苏**债**31"（多一个"债"字），字符串对不上就被误判成缺失。改成"债"字去除后再比对，107条降到**4条**，且这4条全部落在过去一周内、全部在内蒷古/黑龙江（当时都不在覆盖范围内）——符合celma一贯的滞后节奏，不是新问题。**`check_sse_coverage()` 的docstring里现在专门警告了这一条，不要把这个比对改回原始字符串匹配**。

**用户看到"10省已覆盖但SSE缺口对不上"这个矛盾后，明确要求"扩展研究范围"**，去查真正有缺口的省份（当时定位到内蒙古、黑龙江）能不能爬。查证过程：
- **内蒙古自治区**：用Playwright重新走了一遍（首页→法定主动公开→政府债券信息公开栏目），确认栏目本身是真实的、能访问，但内容**只有"关于发行XXX有关事宜的通知"（发行前通知），从来没有"发行结果"类内容**——和第一轮WebFetch时代的结论完全一致，二次确认是真死路，不是当年工具不够强的问题。
- **黑龙江省**：查了很久没查透，让给了下面的并行调研batch继续深挖，最终结论见下。

**鉴于"扩展研究范围"这个要求本身就很大，对第一轮（只用WebSearch+WebFetch，没有JS渲染能力）判定为"没找到/被挡住"的剩余17个省份，用4个并行agent+已验证有效的Playwright方法整体重新调研了一遍**。核心逻辑：第一轮很多"没找到"其实是WebFetch不能执行JS、看不到真实列表内容导致的假阴性，Playwright（真实无头浏览器）理论上能解决一大批。结果证明这个判断是对的——**新确认4个可用省份**：

- **西藏自治区**：`https://www.xizang.gov.cn/zwgk/xxfb/gsgg_428/`（走的是自治区政府综合门户，没有独立的czt.xizang.gov.cn）。第一轮WebFetch在这个具体URL上被网站自己的WAF拦截（"您所提交的请求含有不合法的参数"），Playwright真实浏览器直接就过去了，没做任何特殊配置。内容是干净的内嵌HTML（不走PDF/OCR），实测2支债券100%精确匹配celma。**顺带发现西藏用了第三种招标日期表述**："...财政厅于2026年4月27日在深圳证券交易所招标发行了..."（日期在"招标发行"前面，不是"已完成招标"后面）——加了 `_ISSUE_DATE_RE2` 兜底解析。
- **山东省**：`http://czt.shandong.gov.cn/col/col10559/index.html`。第一轮遇到的"证书不匹配"（证书签给了`*.ybj.shandong.gov.cn`）用 `ignore_https_errors=True` 直接绕过，对真实浏览器来说压根不是问题。**是文本层PDF，不用走OCR**——实测6支债券100%精确匹配celma，是目前所有省级源里质量最好的之一。
- **青岛市**：`http://qdcz.qingdao.gov.cn/zfxxgk/fdzdgknr/zdly/zwgl/index.shtml`。关键发现：**必须用 `http://` 不能用 `https://`**——https协议本身被硬拦截（不是证书问题，`ignore_https_errors=True` 救不了），http完全没有这个问题。第一轮只试了通用市政府门户（只有新闻通稿）和这个域名的https（证书错配到人社局子域名），从没试过http。是扫描件PDF，OCR质量和重庆/天津类似偏低。
- **河南省**：`https://czt.henan.gov.cn/xwdt/tzgg/`（注意不能带 `index_7.html` 这个第7页后缀——第一轮就是被这个后缀带到了旧内容的深分页里，去掉后缀后首页就是最新的）。PDF托管在独立子域名 `xcoss.henan.gov.cn`，OCR质量同样偏低。

**同时确认了12个真正的死胡同**（不是技术限制，是官网自己压根没有"发行结果"内容，逐一验证过每个都不是JS渲染假阴性）：
- **陕西省**：读了PDF全文用pdfplumber搜"票面利率"——全文0命中，通篇只有"拟发行"字样，确认是发行前披露文件，不是结果公告。
- **浙江省**：官网专题栏目"债券信息"栏目下现在**只剩两个外链**——链到celma.org.cn和"中国债券信息网"(chinabond.com.cn) 的地方债信息披露门户，浙江省本级已经不再自己发布这类公告了，全权委托给这两个全国性平台。**这是一个值得记住的新信息源线索**：chinabond.com.cn 有个 `/dfz/#/information/index` 地方债信息披露门户，是SPA页面（`#/`路由），本次没有深挖它的真实后端接口，如果celma+SSE的组合以后还不够用，这是下一个值得用Playwright抓包的候选源。
- **湖北省**：官网栏目齐全（tzgg+zczd两个子栏目全部枚举过），但只有发行计划表和承销团文件，从不发结果公告。
- **江西省、安徽省、广东省**：三个都用Playwright真实渲染确认过内容（安徽/广东还额外解决了下面提到的代理问题），栏目里全部只有发行计划/发行前披露/还本付息类内容，从没出现过"发行结果"字样，WebSearch站内搜索也搜不到——不是抓取失败，是官网真的没发布。
- **黑龙江省**：找到了栏目背后的真实JSON分页接口（`common/search/{channelId}?_isAgg=true&_isJson=true...`，页面上的"下一页"链接是假的`javascript:;`占位符，真实翻页要走这个接口），扫了`部门文件`40页+`政策制度`16页，**最新的一条债券相关文件是"黑财库〔2024〕59号"（2024年12月）**——黑龙江官网大概从2024年底开始就没在自己网站上发布任何债券相关文件了（连发行前通知都没有），但celma上黑龙江2025/2026年的确认结果是有的，说明celma另有渠道拿到这些数据，不是从黑龙江官网抓的。
- **吉林省**：读了最新一条公告全文确认是发行前招标通知（"关于招标发行...的通知"），不是结果公告。
- **云南省**：官网压根没有债券相关栏目（查了政务公开/专题专栏/财政数据/资讯中心四个方向，财政数据下的"政府债务信息公开"栏目只有决算报告，没有发行类内容）。
- **四川省**：第一轮猜的URL本身就是错的（`/scczt/tzgg/`直接403），真实栏目是`/scczt/tzgg/zfxxgk_gknrz.shtml`（需要先访问首页热身再跳转，直接访问会403）——找到后确认内容只有发行前"信息披露"文件（原文写"项目详细情况详见中国债券信息网"，把细节推给了chinabond.com.cn，呼应上面浙江的发现）。

**2个暂时没解决，但明确记录了原因，不是"忘了查"**：
- **山西省**：网络层面就连不上（`ERR_TUNNEL_CONNECTION_FAILED`，`openssl s_client`显示服务器在TLS握手阶段直接断开连接，是比甘肃412更底层的一种拦截），但WebSearch能搜到这个域名2026年的活跃索引页面，说明网站本身是活的——只是这台机器/这个网络环境的出口连不上，不是内容不存在。**如果以后有不同的网络出口（代理/VPN），值得重新试**，调研agent留了一个未验证的候选栏目线索：`czt.shanxi.gov.cn/czdt/rdzt/zfzwgl_49599/xxgk_49600/`。
- **福建省**：确认不是JS渲染问题（Playwright完整渲染了页面），是服务器端的一个分类过滤bug——页面URL带的具体子栏目路径（`dfzfzqfx/`）实际上没有被传进后端真正使用的分类过滤SQL（`classsql`参数固定在更大的`fdzdgknr/%`范围），导致列表内容和URL暗示的栏目完全不对应；侧边栏分类链接是无障碍工具栏注入的假`javascript:;`节点，点了没反应，真实的分类切换逻辑在一个没有暴露内联事件的JS框架里，没能在预算内逆向出来。**这个需要专门再开一轮才能解决，不建议顺带做**。

**顺带发现一个环境层面的坑，会在以后调研新省份时反复遇到**：这台机器有一个系统级HTTP/HTTPS代理（`127.0.0.1:1082`，`scutil --proxy`能看到，不在环境变量里），对安徽/广东两个域名会导致Playwright连接直接失败（`ERR_TUNNEL_CONNECTION_FAILED`），而`curl`直连完全没问题。**修复方法是给Playwright的 `browser.new_context()` 显式指定 `proxy={"server": "127.0.0.1:1082", "bypass": "目标域名"}`，强制该域名走直连**——目前只在江西/安徽/广东的调研脚本里手动加过，还没有沉淀进 `provincial_crawl.py` 的正式代码里（因为这三个省份最后都确认是死胡同，没有实际接入的必要），**如果以后新增的省份也遇到同样的 `ERR_TUNNEL_CONNECTION_FAILED`，直接按这个模式加代理绕过，不用重新排查**。

**最终结果：10省扩展到14省，全部用真实数据端到端测试过**（`crawl_all(months_back=2)`）：25支债券干净匹配，0处真实金额/利率差异，12处因源端信息不全无法匹配（多数是重庆/青岛/河南的OCR质量问题），8处被低置信度防护正确拦下。所有新代码改动都已经跑过 `main.py --skip-crawl`（默认路径，非skip-provincial）确认能正常跑完整流程且幂等。
...
### 2026-08-12 第四轮：OCR质量突破（全项目通用）+ 发行计划新解析器（6/10省补齐）

用户指令："给chinabond的'发行计划'栏目写个新解析器"，对应第三轮末尾记的"下一步①"。

**① OCR质量突破，顺带发现、全项目生效**：调试内蒙古一份"用眼睛能看清但pytesseract读不出表格"的PDF时，系统性对比了渲染后端和Tesseract参数两个维度，发现两个独立但都关键的改进点：
1. 渲染改用 `pdfplumber` 自带的 `page.to_image()`（调色板模式 PIL 图），**不用** PyMuPDF/`fitz` 的 `get_pixmap()`（RGB模式）——同分辨率下前者OCR效果明显更好，已确认和色彩模式本身无关（P模式和强制转RGB模式效果一样好，问题出在fitz渲染器本身，不是色深）。
2. Tesseract 参数改用 `--psm 4`（"假设是单栏、多种字号的文本"），**不用**默认的 `--psm 3`（全自动版面分析，在"表格边框+红色公章遮挡"这种排版上会整体错乱）。
3. **反直觉发现**：分辨率从300 DPI降到150 DPI反而效果更好（配合上述渲染器）——字号相对Tesseract的预期范围变得更合适。两个维度必须一起测，不能只调一个就下结论。

已把这个组合固化进 `src/pdf_extract.py::_ocr_pages()`（现在完全不依赖 `fitz`，import 都删了），**这是全项目通用的默认行为，不只是给发行计划用**——发行结果的OCR也会自动受益。验证过对上海计划PDF、内蒙古/河南/重庆结果PDF等5份此前识别率低的文档，重庆那份从"0/6条债券可读"变成"几乎全部字段可读"。**如果以后发现某份文档在这个组合下反而变差，值得单独调查，不要不经重新测试就把默认值改回去**（见 `pdf_extract.py` 里的详细注释）。

**② 发行计划新解析器**：`src/chinabond_crawl.py` 新增 `parse_plan_pdf()` 及其辅助函数，三层解析策略（因为实地确认至少有4种不同的表格模板在用）：
- 结构化表格解析（两种方向）：`_parse_plan_table_column_oriented()` 处理"山东/辽宁式"（新增/再融资是列组表头，一省一数据行）；`_parse_plan_table_row_oriented()` 处理"广东式"（新增/再融资是行组标签，专门有一列"合计"存周期总额）。
- `_parse_plan_text_fallback()`：文本层里找"XX债券"紧邻的数字（表格结构丢失但文字层还在时用）。
- `_parse_plan_text_positional()`：连分类标签都丢了时（OCR太差），从"合计"这一行按固定顺序数7个数字，**必须先算术自洽校验通过才采信**（新增一般+新增专项≈新增小计 等，容差 `max(0.5, 总额*1%)`），校验不过就返回空，绝不瞎猜。

顺手修了两个实测中发现的真bug：① `_parse_plan_table_column_oriented()` 一开始漏判辽宁（地区标签是"辽宁**地区**"，不是"XX省/市/自治区"后缀，加了"地区"关键词，同时排除表头单元格自身的"地区"两个字精确匹配）；② OCR会偶发在小数点后插入一个多余空格（"27. 0274"读成"27."和"0274"两段），导致 `_parse_plan_text_positional()` 数字错位、校验失败——加了 `re.sub(r"(\d)\.\s+(\d)", r"\1.\2", ...)` 归一化（这个artifact类型 `extract_result.py::_to_float()` 里之前就修过一次，这次是同一个坑在新代码路径里重现）。另外 `src/extract_plan.py::TITLE_QUARTER_RE` 放宽了年份和季度标记之间的间隔（原来要求紧邻，chinabond广东标题是"2026年广东省政府债券三季度"，年份和"季度"之间夹着发行人名称）。

**③ 实测结果**：对第三轮记录的"8月缺发行计划的10个省份"逐一测试，**6个成功补齐**：上海市（一般108.0/专项140.79亿元）、吉林省（一般21.0/专项33.0/再融资10.0亿元）、广东省（一般122.1/专项1868.8/再融资281.1亿元，三季度口径）、辽宁省、山西省、新疆生产建设兵团（均为三季度口径，部分子项因表格模板问题暂未拆出细项但总量已可读）。**剩下4个（大连市、河南省、西藏自治区、青岛市）不是解析器问题**——用长短两种发行人名称格式都直接查过chinabond的API，这4个省份/城市在这个栏目下**目前就是没有近期数据**，是数据源本身的缺口，不是我们读不出来。

`crawl_plans()`/`crawl_all_plans()` 和第三轮的 `check_chinabond_for_sse_gaps()` 一样，**故意没有接入 `main.py` 默认流程**——同样出于"OCR提取结果需要人工抽查，不能盲目自动patch进state_plans.csv"的考虑，保持为按需调用的诊断工具。如果以后要接入，这两个函数是现成的起点。

### 2026-08-13 第五轮：人工核对chinabond数据并实际写入两张state表（含两个真实bug）

用户先问"现在输出结果如何？与Wind还有差异吗？"，答完后进一步要求"现在核对并写入"——把第四轮"已确认文档存在、能读出数字，但还没写入CSV"的数据真正落库。这一轮的核心是**边核对边发现了两个之前没暴露的真实bug**，都是在"必须先肉眼核对再写入"这条规矩下才抓到的。

**① 重新对账**：仓库根目录躺着一份没处理过的Wind导出`地方政府债2026-08-11.xlsx`，跑了`wind_reconcile.reconcile_from_wind_file(dry_run=True)`——15条"缺失"全部是2025-08~2026-05的旧数据（26甘肃债03~13一整批等），近2个月依然0差异，结论不变。

**② 吉林发行计划的数字是错的——`_parse_plan_text_fallback()`有真bug**：渲染吉林那份计划PDF人眼核对时发现，代码给出的"21.0/33.0/10.0"和图上的"21.3000/33.5400/32.1660(再融资一般)+10.2112(再融资专项)"对不上——差了两处：(a) OCR的"21. 3000"小数点后有个多余空格，这个fallback函数没做归一化（`_parse_plan_text_positional`早就修过这个artifact，但没同步到这个函数）；(b) 原来按"再融资债券"这个标签的文本位置做前后切分，但吉林这份OCR文本里"一般债券 32.1660"这个数字实际排在"再融资债券"这四个字**前面**（读取顺序错位），导致再融资-一般这个字段被完全漏掉。改成按"一般债券"/"专项债券"在全文里出现的**顺序**取值（第1次=新增，第2次=再融资），不再依赖标签切分位置，重新核对上海+吉林两份文档完全吻合。**教训**：OCR文本的"标签在数字前面"这个假设不能全局信任，哪怕能过自洽性测试也可能悄悄错，这次是靠人眼核对图片才抓到的，不是代码自己发现的。

**③ SSE缺口反查另一个更隐蔽的bug——`check_sse_coverage()`省份简称不完整**：核对内蒙古/黑龙江那4条"缺口"时发现它们**其实早就在`state_results.csv`里**（黑龙江2条是celma自己发布的原生数据，内蒙古2条是更早一轮Wind人工补录的）——SSE把"黑龙江省"简称成"龙江"、"内蒙古自治区"简称成"内蒙"（各自砍掉了根名称的一个字，不是常规的砍"省/自治区"后缀），celma自己从不这么简称，之前"去掉债字"的归一化没覆盖这种情况。这两个省份是`config.PROVINCES`里仅有的根名称3字以上的省份，是个封闭集合，加了一个别名映射直接解决。**修复后，近2个月SSE缺口从11条降到7条（河南4+广东3），另外4条根本不是缺口**。这是继"债"字归一化bug之后同一类问题的第二次复现，提醒以后遇到"一个省份的比对结果显得异常"时，第一反应应该是去核对命名规则，而不是先假设数据真的缺失。

**④ 实际写入**：
- **发行计划**（`state_plans.csv` +6行）：上海/吉林/广东/辽宁/山西/新疆生产建设兵团2026年8月/三季度计划，吉林用了修复后的正确数字。全部先渲染成图片肉眼核对过一遍（新增：山西那份"合计"行的算术自校验今天也补做了肉眼复核，确认517=344+173、344=120+224、173=29+144，和图上完全一致）。
- **发行结果**（`state_results.csv` +7行）：河南58~61（26河南债58~61，均10年/1.76%，58是新增专项44.52亿，59/60/61实为"再融资专项债券十八/十九/二十期"分别193.67/18.06/44.56亿）、广东44~46（26广东债44~46，10年一般75.7482亿/1.74%、5年专项150.6767亿/1.45%、7年专项92.1901亿/1.57%）。两份来源PDF都渲染成图片肉眼逐字段核对过（河南是带红色公章的正式扫描件，广东是文本原生表格，两者可靠性都高于自动OCR/正则提取）。**`bond_code`留空**：chinabond PDF上印的"债券编码"（河南199659~199662、广东199663~199665）是交易所/托管编码，和celma惯用的中债登`260xxxx`编码不是同一套体系，写进去会造成两套编码混用，宁可留空也不要猜。
- 写入后重跑`check_sse_coverage()`确认：近2个月SSE缺口降为**0**。

### 2026-08-13 第六轮：新增大连市为第15个省级数据源

用户问"这四个省份（大连/河南/西藏/青岛）有财政局官网吗"（针对第五轮里那4个chinabond查不到8月计划的地方）。答案：河南/西藏/青岛的官网早就接入了，往前查6个月也没有独立的"发行计划"文档，和chinabond的缺口是同一个事实；但**大连市财政局官网`czj.dl.gov.cn`是全新发现，之前项目里大连完全没有独立渠道**。用户接着说"试试看"，于是正式接入：

- 列表栏目 `https://czj.dl.gov.cn/col/col5025/index.html`（"财政公告"，和"信息披露文件"混在一起，靠已有的`RESULT_KEYWORDS`过滤）。
- **结构是全项目目前最干净的一类**：内联HTML，标准债券名称/计划发行规模/...模板直接在页面正文里，完全不需要PDF/OCR。用2026年第五批（2026-06-25，9支债券）实测，`parse_announcement_text()`直接吃现成文本，9/9全部字段（规模/期限/利率/招标日）和celma库里的26大连债14~22逐一核对完全一致。
- 同样往前查了6个月的列表，确认大连市自己的官网也**没有独立的"发行计划"栏目**——和河南/西藏/青岛一样，这不是抓取能力问题，是这4个地方压根不通过自己的官网发布这类文档。
- 已加入`provincial_verify.py::PROVINCE_SOURCES`（第15个省份）和`provincial_crawl.py::LISTING_URLS`，`three_way_validate.crawl_all()`不需要改代码就会自动把大连纳入下次的三方校验（它读的是`LISTING_URLS.keys()`，不是写死的省份列表）。
...
- **chinabond.com.cn 相关工作现状**：① 发行计划解析器（第四轮完成）、OCR质量问题（第四轮解决，全项目生效）、②当时列的"人工抽查校验后patch进state表"（第五轮已完成：7条发行结果+6条发行计划已人工核对写入，过程中还顺带修了两个真bug，见第五轮记录）都已经做完；③ chinabond的翻页参数还没找到（`pageSize` 目前只能拿到每个筛选条件下最新的10条），如果要做历史数据回填而不只是"补最近缺口"，需要先破解翻页——这是目前唯一还没碰过的方向。今后每次跑`check_sse_coverage()`如果又看到河南/广东以外的省份出现"新缺口"，直接照第五轮的流程走（chinabond找文档→渲染成图片肉眼核对→写入），不用重新摸索。
...
| GitHub 仓库 | `fletcherfeng1919-wq/local-bond-announcements`，分支 `main`，最新 commit `c2cab08` |
