...
**没做完的部分——下个会话大概率会被继续问**：看板里其余约30个图表（收益率曲线、关键期限、券种利差、区域利差、地方债vs国债利差、发行规模趋势等等）的数字，全部是本项目早期会话里用一次性pandas脚本算出来再手写进JS里的**硬编码值**，不是像日历/计划那样实时读CSV算出来的——这次修的 `state_results.csv` 虽然更准了，但这些硬编码图表**还没有跟着重新计算**，尤其是涉及2025-08~2026-08这个窗口的数字（比如"关键期限时间序列"2025/2026年的中位数、"发行规模趋势"2025年的分类汇总等）理论上都会有小幅变化。已经明确告知用户这一点，如果用户下次问"图表怎么还是老数字"或要求"把图表也重新算一遍"，这就是要继续做的事——工作量不小（要把本次会话之前几十轮里做过的所有统计脚本重新跑一遍），开工前建议先问用户是否真的需要全量重算，还是只挑几个受影响最大的图表改。
