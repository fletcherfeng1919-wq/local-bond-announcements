...
**当前真正待办、下个会话大概率会被继续问的事**：把上面研究出来的10个"已确认可用"的省级财政厅/财政局官网（上海/新疆/天津/河北/贵州/宁夏/湖南/重庆/江苏/宁波）接入一个可复用的交叉验证模块（比如叫 `src/provincial_verify.py`），逻辑上应该长得像 `wind_reconcile.py`（同样是"外部源 vs `state_results.csv` 逐笔diff"的模式），但注意**三种不同的页面结构，不能用同一套解析代码**：
1. 封面页+PDF（上海/新疆/河北/贵州/江苏）——复用现有的 `src/pdf_extract.py` 应该就够。
2. 直接内嵌HTML表格（宁夏/宁波）——不需要PDF提取，直接解析HTML `<table>`，反而是最容易做的两个。
3. 封面页+图片（天津）——需要OCR，比PDF更麻烦，可以先跳过或放最后做。
4. 封面页+.docx（湖南）——需要一个docx文本提取库（`python-docx`），项目目前没有这个依赖。

用户明确说了"你不需要把数据不完整的省份也刷新"（原话见上一节引用）——**不要主动去把"部分确认"和"完全没找到"的那17个省份继续调查到底**，除非用户明确要求。当前这10个已经够开始建模块的了，且用户的核心需求是"最近两个月+当前月"的滚动交叉验证，不是历史全量覆盖，所以模块设计上应该聚焦"给定一个省份+月份，抓当月的发行结果公告并解析"这个函数，而不是一次性抓全部历史。
